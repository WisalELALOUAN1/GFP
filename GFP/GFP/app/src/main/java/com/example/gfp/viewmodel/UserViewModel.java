...
public class UserViewModel extends ViewModel {
        ...
        if (isNetworkAvailable(context)) {
            ...
            authRepo.login(email, pwd, new AuthRepository.AuthCallback() {
                ...
                public void onFailure(String message) {
                    // Si Firebase échoue, essayer avec Realm
                    tryLocalLogin(email, pwd, cb);
                }
            });
        } else {
            // Pas de réseau, essayer directement avec Realm
            tryLocalLogin(email, pwd, cb);
        }
    }
        ...
        try {
            realm.executeTransaction(r -> {
                ...
                if (stored == null) {
                    ...
                    newUser.setPassword(""); // Note: Ne stockez pas les mots de passe en clair en production
    ...
    private void tryLocalLogin(String email, String pwd, AuthRepository.AuthCallback cb) {
        ...
        try {
            ...
            if (user != null) {
                ...
                cb.onSuccess(null); // Passer null car c'est un login local
            } else {
    ...
    public void addGoalToUser(User user, Goal goal) {
        // Effectuer l'ajout de l'objectif dans la transaction Realm
        Realm realm = Realm.getDefaultInstance();
        realm.executeTransaction(r -> {
            ...
            r.insertOrUpdate(user); // Mettre à jour l'utilisateur avec le nouvel objectif
            Log.d("UserViewModel", "Objectif ajouté : " + goal.getDescription());
        });
        goalsLiveData.setValue(user.getGoals()); // Mise à jour des données
    }
    ...
    // Ajouter la méthode getGoalById
